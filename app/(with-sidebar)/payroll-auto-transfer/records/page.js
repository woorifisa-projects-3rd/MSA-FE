...
export default function PayRecords() {
    ...
    const filteredList = list.filter(item => {
        ...
        return itemDate.getFullYear() === selectedDate.getFullYear() && itemDate.getMonth() === selectedDate.getMonth() && item.name.includes(searchQuery);
    });
    ...
    const updatedList = sortedList.map(item => {
        ...
        const updatedAccount = bank ? (
            <span style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <div className={classes.bankIcon} dangerouslySetInnerHTML={{ __html: bank.logoUrl }} />
